...
describe("atlas layer table", () => {
  it("gives every block face a layer, shares identical face keys, air has none", () => {
    ...
    expect(distinct.size).toBe(9 + 7 + 6 + 1 + 5); // 9 wall tiles + dirt, grass_top, grass_side, sand, rock, ore, bedrock + a side and a top for each station + water + log, log_top, leaves, planks, sapling
  });
});
