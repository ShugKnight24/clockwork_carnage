...
import { BLOCKS, AIR, BEDROCK, isTargetable } from "../../src/world/blocks.js";

describe("gather table", () => {
  it("covers every block the pick can target except bedrock", () => {
    const covered = Object.keys(GATHER).map(Number);
    // Water is not mined: it is scooped with a bucket. A sapling is not solid
    // but is targeted, so it can be dug up.
    const expected = BLOCKS.filter((b) => b.id !== AIR && b.id !== BEDROCK && isTargetable(b.id)).map((b) => b.id);
