...
out vec3 v_world; out float v_layer; out float v_normal;
const float SURFACE_DROP = ${(1 - WATER_SURFACE).toFixed(4)};
void main() {
  vec3 p = a_pos + u_origin;
  if (a_ao > 0.5) p.z -= SURFACE_DROP;
  v_world = p; v_layer = a_layer; v_normal = a_normal;
  gl_Position = u_viewProj * vec4(p, 1.0);
}`;

export const WATER_FRAG = `#version 300 es
precision highp float; precision highp sampler2DArray;
in vec3 v_world; in float v_layer; in float v_normal;
uniform sampler2DArray u_atlas;
...
void main() {
  ...
  vec2 q = n >= 4 ? v_world.xy : vec2(v_world.x + v_world.y, v_world.z);
  float r1 = ripple(q * 0.21 + vec2(0.031, 0.019) * u_time);
  float r2 = ripple(q.yx * 0.29 + vec2(-0.023, 0.036) * u_time);
  float rip = 0.5 * (r1 + r2);
  ...
  float fog = u_fogMax * (1.0 - exp(-dcam * u_fogDensity));
  vec3 fogC = mix(u_fogNear, u_fogFar, clamp(fog / max(u_fogMax, 1e-3), 0.0, 1.0));
  o_color = vec4(mix(col, fogC, fog), alpha);
  o_depth = vec4(0.0);
}`;
