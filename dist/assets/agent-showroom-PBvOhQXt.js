...
`,I=class extends HTMLElement{constructor(){super(),this.game=null,this.category=0,this.preview=null,this.previewSection=null,this.undoStack=[],this.snapshot=null,this.session=!1,this.isOpen=!1,this.poseChoice=`idle`,this.cameraChoice=`full`,this.helmetOff=!1,this.yaw=0,this.yawTarget=0,this.view=r.full.slice(),this._stageCache=new Map,this._raf=0,this._renderQueued=!1,this._thumbSeq=0;let e=this.attachShadow({mode:`open`});e.innerHTML=`<style>${F}</style>${this.template()}`;let t=t=>e.querySelector(t);this.$=t,this.el={root:t(`.root`),stage:t(`.stage`),figure:t(`.figure`),figureWrap:t(`.figure-wrap`),callsign:t(`.callsign`),sub:t(`.sub`),tabs:[...e.querySelectorAll(`.tab`)],content:t(`.content`),save:t(`.save`),back:t(`.back`),undo:t(`[data-act="undo"]`),toast:t(`.toast`),confirm:t(`.confirm`),presets:t(`.presets`),live:t(`.live`)},this.bind()}template(){let e=k.map((e,t)=>`<button class="tab" role="tab" id="tab-${e.id}" aria-controls="panel" aria-selected="false" data-cat="${t}" tabindex="-1">${O[e.id]}<span>${e.label}</span></button>`).join(``),t=D.map((e,t)=>`<button class="preset" data-preset="${t}" aria-label="Apply ${e.name} preset"><span class="pthumb"></span><span>${e.name}</span></button>`).join(``);return`
<div class="root">
  ...
  <div class="confirm" hidden>
    <div class="dialog" role="alertdialog" aria-modal="true" aria-labelledby="cf-t" aria-describedby="cf-d">
      <div class="frame"><div class="body">
        ...
        <p class="err ${n?``:`ok`}" id="callsign-err">${A(n||`Letters, numbers, space . _ ' -`)}</p></div>`}let t=e.data.map((t,n)=>this.optionHtml(e,t,n)).join(``),n=e.kind===`rifle`?`<div class="rifle-hero" aria-hidden="true"><div class="spin"></div></div>`:``;return`<div class="section" data-key="${e.key}"><h3><span>${e.title}</span><span class="cur"></span></h3>${n}<div class="grid ${e.kind}" role="radiogroup" aria-label="${e.title}">${t}</div>${this.lockInfoHtml(e)}</div>`}lockInfoHtml(t){if(!a[t.key]||t.kind===`class`)return``;let n=this.unlockCtx(),r=new Map;return t.data.forEach((i,a)=>{let o=e(t.key,a,n);if(o.unlocked)return;let s=r.get(o.hint)||{st:o,names:[]};s.names.push(i.name),r.set(o.hint,s)}),r.size?`<div class="lockinfo">${[...r.values()].map(({st:e,names:t})=>`<div class="lockrow"><span class="lock caption amber">${O.lock}${A(t.join(`, `))}</span><span>${A(e.hint)}</span><span class="bar" aria-hidden="true"><i style="--w:${Math.round(e.pct*100)}%"></i></span></div>`).join(``)}</div>`:``}optionHtml(t,n,r){let i=a[t.key]?e(t.key,r,this.unlockCtx()):null,s=!!i&&!i.unlocked,c=`class="opt plate${t.kind===`origin`||t.kind===`voice`||t.kind===`class`?` card`:``}${s?` locked`:``}" role="radio" aria-checked="false" data-key="${t.key}" data-idx="${r}" tabindex="-1"`,l=`${n.name}${n.desc?`. ${n.desc}`:``}${n.perk?`. Perk: ${n.perk}`:``}${s?`. Locked: ${i.hint}`:``}`,u=`<span class="tick" aria-hidden="true">${O.check}</span>`,d=n.tier?`<span class="tier" aria-hidden="true">${[1,2,3].map(e=>`<i class="${e<=n.tier?`on`:``}"></i>`).join(``)}&nbsp;${M[n.tier]}</span>`:``;switch(t.kind){case`eye`:return`<button ${c} aria-label="${A(l)}" title="${A(n.name)}"><span class="swatch" style="--c:${n.color}"></span><span class="oname">${A(n.name)}</span></button>`;case`skin`:return`<button ${c} aria-label="${A(l)}" title="${A(n.name)}"><span class="swatch" style="background:radial-gradient(circle at 35% 30%, ${n.color}, ${n.shadow})"></span><span class="oname">${A(n.name)}</span></button>`;case`palette`:return`<button ${c} aria-label="${A(l)}">${u}<span class="pal" aria-hidden="true"><i style="background:linear-gradient(135deg, ${n.primary}, ${n.dark})"></i><i style="background:${n.accent}"></i><i style="background:${n.dark}"></i></span><span class="oname">${A(n.name)}</span></button>`;case`badge`:return`<button ${c} aria-label="${A(l)}" title="${A(n.name)}">${u}<span class="badge-tile" aria-hidden="true">${n.icon?`<svg viewBox="-6.5 -6.5 13 13">${o(n.icon,`var(--accent)`,`#070b11`)}</svg>`:`<span class="oname" style="color:var(--faint)">None</span>`}</span><span class="oname">${A(n.name)}</span></button>`;case`origin`:return`<button ${c} aria-label="${A(l)}">${u}<span class="row"><span class="oname">${A(n.name)}</span></span><span class="desc">${A(n.desc)}</span><span class="perk caption cyan">${A(n.perk)}</span></button>`;case`voice`:return`<div class="voice-row" style="position:relative"><button ${c} aria-label="${A(l)}" style="width:100%;padding-right:60px">${u}<span class="oname">${A(n.name)}</span><span class="desc">${A(n.desc)}</span></button><button class="play" data-voice="${r}" aria-label="Preview ${A(n.name)} voice" style="position:absolute;right:10px;top:50%;transform:translateY(-50%)">${O.play}</button></div>`;case`class`:{let e=N(n).map(e=>{let t=t=>`${Math.round(j(t/e.max,0,1)*100)}%`,n=e.fmt||(e=>Math.round(e)),r=e.value-e.base,i=Math.abs(r)<1e-6?``:r>0?`up`:`down`;return`<span>${e.label}</span><span class="bar"><i style="--w:${t(e.value)}"></i></span><b class="${i}">${n(e.value)}</b>`}).join(``),t=s?`<span class="lock caption amber">${O.lock}Locked</span>`:``,r=s?`<span class="lockprog"><span>${A(i.hint)}</span><span class="bar" style="width:70px" aria-hidden="true"><i style="--w:${Math.round(i.pct*100)}%"></i></span></span>`:``;return`<button ${c} aria-label="${A(l)}" aria-disabled="${s}">${u}<span class="row"><span class="oname">${A(n.name)}</span>${t}</span><span class="desc">${A(n.desc)}</span>${r}<span class="stats" aria-hidden="true">${e}</span></button>`}case`rifle`:return`<button ${c} aria-label="${A(l)}">${u}<span class="thumb" data-thumb="rifle"></span><span class="oname">${A(n.name)}</span></button>`;default:return`<button ${c} aria-label="${A(l)}"${s?` aria-disabled="true" title="${A(i.hint)}"`:``}>${u}${s?`<span class="lockbadge" aria-hidden="true">${O.lock}</span>`:``}<span class="thumb" data-thumb="${t.kind}"></span><span class="oname">${A(n.name)}</span>${d}</button>`}}refreshContent(e=!1){let n=this.character,a=S.map(e=>n[e]).join(`,`);this.shadowRoot.querySelectorAll(`.content .opt[data-key]`).forEach(o=>{let s=o.dataset.key,c=Number(o.dataset.idx);o.setAttribute(`aria-checked`,String(n[s]===c));let l=o.querySelector(`.thumb[data-thumb]`);if(!l||!e&&l.dataset.sig===a)return;l.dataset.sig=a;let u={...n,[s]:c},d=`t${this._thumbSeq++}-`;switch(l.dataset.thumb){case`head`:l.innerHTML=t(u,{headOnly:!0,idPrefix:d});break;case`hair`:l.innerHTML=t(u,{headOnly:!0,peek:!0,idPrefix:d});break;case`torso`:l.innerHTML=t(u,{view:r.torso,idPrefix:d,lighting:`flat`});break;case`rifle`:l.innerHTML=i(u,{idPrefix:d});break;default:}});let o=this.shadowRoot.querySelector(`.rifle-hero .spin`);o&&(e||o.dataset.sig!==a)&&(o.dataset.sig=a,o.innerHTML=i(n,{idPrefix:`rh${this._thumbSeq++}-`}));let s=this.shadowRoot.querySelector(`input[name=callsign]`);s&&this.shadowRoot.activeElement!==s&&s.value!==(n.name||``)&&(s.value=n.name||``),this.el.save.setAttribute(`aria-disabled`,String(!!this.nameError(n.name||``))),this.renderPreviewLabels(),this.el.undo.disabled=this.undoStack.length===0}toast(e){let t=this.el.toast;t.textContent=e,t.classList.add(`on`),clearTimeout(this._toastT),this._toastT=setTimeout(()=>t.classList.remove(`on`),2200)}announce(e){this.el.live.textContent=e}};customElements.get(`agent-showroom`)||customElements.define(`agent-showroom`,I);function L(e){let t=document.querySelector(`agent-showroom`);return t||(t=document.createElement(`agent-showroom`),document.body.appendChild(t)),t.game=e,t}export{L as mountShowroom};