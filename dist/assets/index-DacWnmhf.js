const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/cutscene-mcIzaefv.js","assets/data-i2TjbbKC.js","assets/builder-CY5RIlBf.js","assets/analytics-5fjFOfxM.js"])))=>i.map(i=>d[i]);
import{i as e,n as t,r as n,t as r}from"./analytics-5fjFOfxM.js";import{C as i,S as a,T as o,_ as s,a as c,b as l,c as u,d,f,g as p,h as m,i as h,l as g,m as _,n as v,o as y,p as b,r as x,s as S,t as C,u as w,v as T,w as E,x as D,y as O}from"./data-i2TjbbKC.js";import{_ as k,a as A,c as j,d as M,f as N,g as P,h as F,i as ee,l as te,m as ne,n as I,o as re,p as ie,r as ae,s as oe,t as se,u as ce}from"./enemy-renderers-D7d6X9kW.js";(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var le=class{constructor(e){this.game=e,this.active=!1,this.selectedCategory=`enemies`,this.selectedId=null,this.tweakData={},this.container=null,this.initUI()}initUI(){this.container=document.createElement(`div`),this.container.id=`asset-editor-overlay`,this.container.style.cssText=`
            position: absolute;
            top: 20px;
            right: 20px;
            width: 320px;
...
Note: Only Safari supports home screen apps on iOS.`)}))}else $.style.display=`none`;else if(ac)$.style.display=`none`;else{$.addEventListener(`click`,async()=>{try{if(document.fullscreenElement||document.webkitFullscreenElement){let e=document.exitFullscreen||document.webkitExitFullscreen;e&&await e.call(document)}else{let e=document.documentElement,t=e.requestFullscreen||e.webkitRequestFullscreen;t&&await t.call(e)}}catch{}});let e=()=>{$.textContent=document.fullscreenElement||document.webkitFullscreenElement?`⛶ EXIT FULLSCREEN`:`⛶ FULLSCREEN`};document.addEventListener(`fullscreenchange`,e),document.addEventListener(`webkitfullscreenchange`,e),e()}qs.addEventListener(`click`,()=>{Q(),X.audio.menuConfirm(),X.loadCampaignSave()?Z():ec()}),Ys.addEventListener(`click`,()=>{Q(),X.audio.menuConfirm(),X.loadArena()?Z():ec()}),Gs.addEventListener(`click`,()=>{Q(),X.audio.menuConfirm(),tc()}),document.addEventListener(`keydown`,e=>{if(X.state===J.TITLE&&(e.code===`Enter`||e.code===`Space`||e.code===`GamepadStart`)){Q(),X.audio.menuConfirm(),tc();return}if(X.state===J.MODE_SELECT){if(e.code===`Digit1`)document.getElementById(`btnCampaign`).click();else if(e.code===`Digit2`)document.getElementById(`btnMeltdown`).click();else if(e.code===`Digit3`)document.getElementById(`btnArena`).click();else if(e.code===`Digit4`)document.getElementById(`btnTutorial`).click();else if(e.code===`Digit5`)document.getElementById(`btnBuilder`).click();else if(e.code===`Digit6`)document.getElementById(`btnCustomize`).click();else if(e.code===`Digit7`)document.getElementById(`btnStats`).click();else if(e.code===`Digit8`)document.getElementById(`btnArchive`).click();else if(e.code===`Escape`)document.getElementById(`btnBack`).click();else if(e.code===`ArrowUp`||e.code===`ArrowDown`||e.code===`KeyW`||e.code===`KeyS`){let t=Array.from(Ks.querySelectorAll(`.mode-btn:not(.hidden)`));if(t.length===0)return;let n=t.indexOf(document.activeElement),r=e.code===`ArrowUp`||e.code===`KeyW`?-1:1,i;i=n===-1?r===1?0:t.length-1:(n+r+t.length)%t.length,t[i].focus(),X.audio.menuSelect()}else if(e.code===`Enter`||e.code===`Space`){let e=document.activeElement;e&&e.classList.contains(`mode-btn`)&&e.click()}}(X.state===J.GAME_OVER||X.state===J.VICTORY)&&(e.code===`Enter`||e.code===`Space`)&&(Gs.classList.remove(`hidden`),Ks.classList.add(`hidden`))});var oc=null,sc=0,cc=0,lc=0;function uc(e){try{let t=Hs||X.showFPS?performance.now():0;X.update(e);let n=X.settings.batterySaver?30:[0,30,60,90,120][X.settings.frameTarget]||0;if(!(!n||!lc||e-lc>=1e3/n)){requestAnimationFrame(uc);return}lc=e;let r=t?performance.now()-t:X.deltaTime*1e3;X.state!==oc&&(oc=X.state,X.state===J.TITLE?(Gs.classList.remove(`hidden`),Ks.classList.add(`hidden`),Us.style.display=`none`,Ws.style.display=`none`):X.state===J.MODE_SELECT?(Gs.classList.add(`hidden`),Ks.classList.remove(`hidden`),ec(),Us.style.display=`none`,Ws.style.display=`none`):(Gs.classList.add(`hidden`),Ks.classList.add(`hidden`),Us.style.display=`block`,Ws.style.display=`block`));let i=0;if(X.state!==J.TITLE&&X.state!==J.MODE_SELECT){let e=Hs||X.showFPS?performance.now():0;if(X.render(),i=e?performance.now()-e:0,X.transitioning&&X.transitionAlpha>0){let e=X.hudCtx;X._renderTransitionOverlay(e,X.hudW,X.hudH);let t=X.renderer.ctx;X._renderTransitionOverlay(t,Us.width,Us.height)}}(Hs||X.showFPS)&&X.profiler.recordFrame(r,i,X.entities.length);let a=cc?e-cc:1e3/60;if(cc=e,Zs.recordFPS(1e3/Math.max(a,1)),Zs.adjust(e)&&(X.applyPerformanceSettings(),Zs.stableScale=Zs.renderScale,$s()),X.showFPS){let e=X.hudCtx;X.profiler.render(e,4,4,220,280)}dc&&dc.render(),sc=0}catch(e){if(sc++,console.error(`[Clockwork Carnage] Frame error (${sc}):`,e),sc>=60){console.error(`[Clockwork Carnage] Too many consecutive errors, halting game loop.`);return}}requestAnimationFrame(uc)}if(requestAnimationFrame(uc),window.addEventListener(`beforeunload`,()=>{X.state===J.PLAYING&&X.mode===`arena`&&X.saveArena(),X.state===J.PLAYING&&X.mode===`campaign`&&X.saveCampaign()}),window.__ccBeforeUnloadRegistered=!0,window.ccProfiler=()=>X.profiler.getSnapshot(),Hs){let e=`${location.origin}/clockwork_carnage/js/testing/`,t=`${e}harness.js`,n=`${e}debug-bridge.js`,r=`${e}telemetry.js`;As(()=>import(t).then(e=>{window.ccTest=e.createTestRunner(X)}),[]).catch(()=>{}),As(()=>import(n).then(e=>{window.ccDebug=e.createDebugBridge(X)}),[]).catch(()=>{}),As(()=>import(r).then(e=>{window._ccTelemetryModule=e,window.ccTelemetry=e.createTelemetry(X)}),[]).catch(()=>{})}var dc=Rs.init(X);dc&&(X.touchControls=dc);var fc=document.createElement(`template`);fc.innerHTML=`
<style>
  :host {
    display: block;
    position: absolute;
...
`;var pc=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(fc.content.cloneNode(!0))}};customElements.define(`title-background`,pc);var mc=document.createElement(`template`);mc.innerHTML=`
  <style>
    :host {
      display: block;
      position: absolute;
...
`;var hc=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(mc.content.cloneNode(!0))}};customElements.define(`title-clock`,hc);var gc=document.createElement(`template`);gc.innerHTML=`
  <style>
    :host {
      display: block;
      position: absolute;
...
`;var _c=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(gc.content.cloneNode(!0))}};customElements.define(`title-hero`,_c);var vc=document.createElement(`template`);vc.innerHTML=`
  <style>
    :host {
      display: block;
      position: absolute;
...
`;var yc=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(vc.content.cloneNode(!0))}};customElements.define(`title-paradox-lord`,yc);