...
Note: Only Safari supports home screen apps on iOS.`)}))}else $.style.display=`none`;else if(Ls)$.style.display=`none`;else{$.addEventListener(`click`,async()=>{try{if(document.fullscreenElement||document.webkitFullscreenElement){let e=document.exitFullscreen||document.webkitExitFullscreen;e&&await e.call(document)}else{let e=document.documentElement,t=e.requestFullscreen||e.webkitRequestFullscreen;t&&await t.call(e)}}catch{}});let e=()=>{$.textContent=document.fullscreenElement||document.webkitFullscreenElement?`⛶ EXIT FULLSCREEN`:`⛶ FULLSCREEN`};document.addEventListener(`fullscreenchange`,e),document.addEventListener(`webkitfullscreenchange`,e),e()}ws.addEventListener(`click`,()=>{Ns(),Z.audio.menuConfirm(),Z.loadCampaignSave()?Q():js()}),Es.addEventListener(`click`,()=>{Ns(),Z.audio.menuConfirm(),Z.loadArena()?Q():js()}),Ss.addEventListener(`click`,()=>{Ns(),Z.audio.menuConfirm(),Ms()}),document.addEventListener(`keydown`,e=>{if(Z.state===J.TITLE&&(e.code===`Enter`||e.code===`Space`||e.code===`GamepadStart`)){Ns(),Z.audio.menuConfirm(),Ms();return}if(Z.state===J.MODE_SELECT){if(e.code===`Digit1`)document.getElementById(`btnCampaign`).click();else if(e.code===`Digit2`)document.getElementById(`btnMeltdown`).click();else if(e.code===`Digit3`)document.getElementById(`btnArena`).click();else if(e.code===`Digit4`)document.getElementById(`btnTutorial`).click();else if(e.code===`Digit5`)document.getElementById(`btnBuilder`).click();else if(e.code===`Digit6`)document.getElementById(`btnCustomize`).click();else if(e.code===`Digit7`)document.getElementById(`btnStats`).click();else if(e.code===`Escape`)document.getElementById(`btnBack`).click();else if(e.code===`ArrowUp`||e.code===`ArrowDown`||e.code===`KeyW`||e.code===`KeyS`){let t=Array.from(Cs.querySelectorAll(`.mode-btn:not(.hidden)`));if(t.length===0)return;let n=t.indexOf(document.activeElement),r=e.code===`ArrowUp`||e.code===`KeyW`?-1:1,i;i=n===-1?r===1?0:t.length-1:(n+r+t.length)%t.length,t[i].focus(),Z.audio.menuSelect()}else if(e.code===`Enter`||e.code===`Space`){let e=document.activeElement;e&&e.classList.contains(`mode-btn`)&&e.click()}}(Z.state===J.GAME_OVER||Z.state===J.VICTORY)&&(e.code===`Enter`||e.code===`Space`)&&(Ss.classList.remove(`hidden`),Cs.classList.add(`hidden`))});var Rs=null,zs=0,Bs=0,Vs=0;function Hs(e){try{let t=bs||Z.showFPS?performance.now():0;Z.update(e);let n=Z.settings.batterySaver?30:[0,30,60,90,120][Z.settings.frameTarget]||0;if(!(!n||!Vs||e-Vs>=1e3/n)){requestAnimationFrame(Hs);return}Vs=e;let r=t?performance.now()-t:Z.deltaTime*1e3;Z.state!==Rs&&(Rs=Z.state,Z.state===J.TITLE?(Ss.classList.remove(`hidden`),Cs.classList.add(`hidden`),X.style.display=`none`,xs.style.display=`none`):Z.state===J.MODE_SELECT?(Ss.classList.add(`hidden`),Cs.classList.remove(`hidden`),js(),X.style.display=`none`,xs.style.display=`none`):(Ss.classList.add(`hidden`),Cs.classList.add(`hidden`),X.style.display=`block`,xs.style.display=`block`));let i=0;if(Z.state!==J.TITLE&&Z.state!==J.MODE_SELECT){let e=bs||Z.showFPS?performance.now():0;if(Z.render(),i=e?performance.now()-e:0,Z.transitioning&&Z.transitionAlpha>0){let e=Z.hudCtx;Z._renderTransitionOverlay(e,Z.hudW,Z.hudH);let t=Z.renderer.ctx;Z._renderTransitionOverlay(t,X.width,X.height)}}(bs||Z.showFPS)&&Z.profiler.recordFrame(r,i,Z.entities.length);let a=Bs?e-Bs:1e3/60;if(Bs=e,Os.recordFPS(1e3/Math.max(a,1)),Os.adjust(e)&&(Z.applyPerformanceSettings(),Os.stableScale=Os.renderScale,As()),Z.showFPS){let e=Z.hudCtx;Z.profiler.render(e,4,4,220,280)}Us&&Us.render(),zs=0}catch(e){if(zs++,console.error(`[Clockwork Carnage] Frame error (${zs}):`,e),zs>=60){console.error(`[Clockwork Carnage] Too many consecutive errors, halting game loop.`);return}}requestAnimationFrame(Hs)}if(requestAnimationFrame(Hs),window.addEventListener(`beforeunload`,()=>{Z.state===J.PLAYING&&Z.mode===`arena`&&Z.saveArena(),Z.state===J.PLAYING&&Z.mode===`campaign`&&Z.saveCampaign()}),window.__ccBeforeUnloadRegistered=!0,window.ccProfiler=()=>Z.profiler.getSnapshot(),bs){let e=`${location.origin}/clockwork_carnage/js/testing/`,t=`${e}harness.js`,n=`${e}debug-bridge.js`,r=`${e}telemetry.js`;cs(()=>import(t).then(e=>{window.ccTest=e.createTestRunner(Z)}),[]).catch(()=>{}),cs(()=>import(n).then(e=>{window.ccDebug=e.createDebugBridge(Z)}),[]).catch(()=>{}),cs(()=>import(r).then(e=>{window._ccTelemetryModule=e,window.ccTelemetry=e.createTelemetry(Z)}),[]).catch(()=>{})}var Us=gs.init(Z);Us&&(Z.touchControls=Us);var Ws=document.createElement(`template`);Ws.innerHTML=`
<style>
  :host {
    display: block;
    position: absolute;
...
`;var Gs=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(Ws.content.cloneNode(!0))}};customElements.define(`title-background`,Gs);var Ks=document.createElement(`template`);Ks.innerHTML=`
  <style>
    :host {
      display: block;
      position: absolute;
...
`;var qs=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(Ks.content.cloneNode(!0))}};customElements.define(`title-clock`,qs);var Js=document.createElement(`template`);Js.innerHTML=`
  <style>
    :host {
      display: block;
      position: absolute;
...
`;var Ys=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(Js.content.cloneNode(!0))}};customElements.define(`title-hero`,Ys);var Xs=document.createElement(`template`);Xs.innerHTML=`
  <style>
    :host {
      display: block;
      position: absolute;
...
`;var Zs=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(Xs.content.cloneNode(!0))}};customElements.define(`title-paradox-lord`,Zs);