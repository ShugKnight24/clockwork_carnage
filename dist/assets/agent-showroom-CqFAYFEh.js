...
`,I=class extends HTMLElement{constructor(){super(),this.game=null,this.category=0,this.preview=null,this.previewSection=null,this.undoStack=[],this.snapshot=null,this.session=!1,this.isOpen=!1,this.poseChoice=`idle`,this.cameraChoice=`full`,this.helmetOff=!1,this.yaw=0,this.yawTarget=0,this.view=s.full.slice(),this._stageCache=new Map,this._raf=0,this._renderQueued=!1,this._thumbSeq=0;let e=this.attachShadow({mode:`open`});e.innerHTML=`<style>${F}</style>${this.template()}`;let t=t=>e.querySelector(t);this.$=t,this.el={root:t(`.root`),stage:t(`.stage`),figure:t(`.figure`),figureWrap:t(`.figure-wrap`),callsign:t(`.callsign`),sub:t(`.sub`),tabs:[...e.querySelectorAll(`.tab`)],content:t(`.content`),save:t(`.save`),back:t(`.back`),undo:t(`[data-act="undo"]`),toast:t(`.toast`),confirm:t(`.confirm`),presets:t(`.presets`),live:t(`.live`)},this.bind()}template(){let e=k.map((e,t)=>`<button class="tab" role="tab" id="tab-${e.id}" aria-controls="panel" aria-selected="false" data-cat="${t}" tabindex="-1">${O[e.id]}<span>${e.label}</span></button>`).join(``),t=D.map((e,t)=>`<button class="preset" data-preset="${t}" aria-label="Apply ${e.name} preset"><span class="pthumb"></span><span>${e.name}</span></button>`).join(``);return`
<div class="root">
  ...
  <div class="toolbar" role="toolbar" aria-label="Edit">
    <button class="tool" data-act="undo" aria-label="Undo last change">${O.undo}<span class="t">Undo</span><kbd class="key">Z</kbd></button>
    <button class="tool" data-act="reset" aria-label="Reset appearance to standard issue">${O.reset}<span class="t">Reset</span></button>
    <button class="tool" data-act="random" aria-label="Randomize appearance">${O.dice}<span class="t">Randomize</span><kbd class="key">R</kbd></button>
  </div>
  ...
  <div class="stage-tools">
    ...
    <span class="hint-turn" aria-hidden="true">${O.turn}Drag to turn</span>
  </div>
  <aside class="panel" aria-label="Customization">
    <div class="frame"><div class="body">
      <button class="handle" aria-label="Collapse panel" aria-expanded="true">${O.chevron}</button>
...
</div>`}sync(e,t){if(!e){this.isOpen&&this.close(),this.session=!1,this._exitedAt=0;return}this._exitedAt&&performance.now()-this._exitedAt<5e3||(this._exitedAt=0,this.session||this.beginSession(),t&&!this.isOpen?this.open():!t&&this.isOpen&&this.close())}beginSession(){this.session=!0,this.snapshot=this.pick(this.game.character),this.undoStack=[],this.preview=null,this.previewSection=null,this.confirmOpen=!1,this.el.confirm.hidden=!0,this.category=0,this.yaw=this.yawTarget=0,this.poseChoice=`idle`,this.cameraChoice=`full`,this.setAttribute(`camera`,`full`),this.view=s.full.slice(),this._viewTween=null,this._stageKey=null,this.helmetOff=!1,this.shadowRoot.querySelector(`[data-helmet-off]`).setAttribute(`aria-pressed`,`false`),this.shadowRoot.querySelectorAll(`[data-pose]`).forEach(e=>e.setAttribute(`aria-pressed`,String(e.dataset.pose===`idle`))),this.shadowRoot.querySelectorAll(`[data-camera]`).forEach(e=>e.setAttribute(`aria-pressed`,String(e.dataset.camera===`full`))),this.removeAttribute(`sheet`),this.classList.remove(`kbd`);let e=null;try{e=localStorage.getItem(`cc_character`)}catch{}this.returning=!!e,this._focusSaveOnOpen=this.returning}open(){this._unlockCtx=null,this.isOpen=!0,this.setAttribute(`open`,``),requestAnimationFrame(()=>this.setAttribute(`shown`,``)),this.renderAll();let e=this.shadowRoot.querySelector(`.ready`);e.hidden=!this.returning,this.el.save.classList.toggle(`ready`,!!this.returning),this._focusSaveOnOpen&&(this._focusSaveOnOpen=!1,this.el.save.focus({preventScroll:!0})),this._resizeObs??(this._resizeObs=new ResizeObserver(()=>this.onResize())),this._resizeObs.observe(this.el.figureWrap)}close(){this.isOpen=!1,this.shadowRoot.activeElement?.blur?.(),this.removeAttribute(`shown`),this.removeAttribute(`open`),this.preview=null,this._resizeObs?.disconnect(),cancelAnimationFrame(this._raf),this._raf=0}pick(e){let t={};for(let n of C)t[n]=e[n];return t}get character(){return this.game.character}isDirty(){let e=this.character;return C.some(t=>e[t]!==this.snapshot[t])}sfx(e){try{this.game?.audio?.[e]?.()}catch{}}bind(){let e=this.shadowRoot;e.addEventListener(`click`,e=>this.onClick(e)),e.addEventListener(`pointerover`,e=>this.onHover(e,!0)),e.addEventListener(`pointerout`,e=>this.onHover(e,!1)),e.addEventListener(`focusin`,e=>this.onFocus(e,!0)),e.addEventListener(`focusout`,e=>this.onFocus(e,!1)),e.addEventListener(`pointerdown`,()=>this.classList.remove(`kbd`)),e.addEventListener(`keydown`,e=>{e.target?.tagName===`INPUT`&&e.code!==`Tab`&&(e.stopPropagation(),(e.code===`Enter`||e.code===`NumpadEnter`||e.code===`Escape`)&&(e.preventDefault(),e.target.blur(),this.focusNav(this.el.tabs[this.category])))}),e.addEventListener(`input`,e=>{e.target?.name===`callsign`&&this.onName(e.target)});let t=this.el.stage;t.addEventListener(`pointerdown`,e=>{e.button!==0&&e.pointerType===`mouse`||(t.setPointerCapture(e.pointerId),this._drag={x:e.clientX,yaw:this.yawTarget},t.classList.add(`dragging`))}),t.addEventListener(`pointermove`,e=>{this._drag&&this.setYaw(this._drag.yaw+(e.clientX-this._drag.x)/240,!0)});let n=()=>{this._drag=null,t.classList.remove(`dragging`)};t.addEventListener(`pointerup`,n),t.addEventListener(`pointercancel`,n),t.addEventListener(`dblclick`,()=>this.setYaw(0));let r=this.el.content;r.addEventListener(`pointerdown`,e=>{e.pointerType!==`mouse`&&(this._swipe={x:e.clientX,y:e.clientY})}),r.addEventListener(`pointerup`,e=>{let t=this._swipe;if(this._swipe=null,!t)return;let n=e.clientX-t.x;Math.abs(n)>70&&Math.abs(e.clientY-t.y)<40&&(this._swallowClick=!0,this.setCategory(this.category+(n<0?1:-1)))})}onClick(e){let t=e.target.closest?.(`button, [data-idx]`);if(t){if(this._swallowClick){this._swallowClick=!1;return}if(t.dataset.cat!=null)return this.setCategory(Number(t.dataset.cat));if(t.dataset.voice!=null)return this.playVoice(Number(t.dataset.voice),t);if(t.dataset.idx!=null)return this.choose(t.dataset.key,Number(t.dataset.idx),t);if(t.dataset.preset!=null)return this.applyPreset(Number(t.dataset.preset));if(t.dataset.camera)return this.setCamera(t.dataset.camera,!0);if(t.dataset.pose)return this.setPose(t.dataset.pose,!0);if(t.hasAttribute(`data-helmet-off`))return this.toggleHelmet();if(t.dataset.confirm)return t.dataset.confirm===`discard`?this.discard():this.closeConfirm();if(t.classList.contains(`handle`))return this.toggleSheet();if(t===this.el.save)return this.save();if(t===this.el.back)return this.back();switch(t.dataset.act){case`undo`:return this.undo();case`reset`:return this.reset();case`random`:return this.randomize();default:}}}onHover(e,t){if(e.pointerType===`touch`)return;let n=e.target.closest?.(`.opt[data-idx]`);n&&(!t&&n.contains(e.relatedTarget)||this.setPreview(t?{key:n.dataset.key,value:Number(n.dataset.idx)}:null))}onFocus(e,t){let n=e.target.closest?.(`.opt[data-idx]`);n&&this.classList.contains(`kbd`)&&this.setPreview(t?{key:n.dataset.key,value:Number(n.dataset.idx)}:null);let r=e.target.closest?.(`.section`);this.previewSection=t&&r&&r.dataset.key||null}handleKey(e,t){if(!this.isOpen)return!1;let n=!!(t&&typeof t.preventDefault==`function`),r=()=>(n&&(t.preventDefault(),t.stopImmediatePropagation()),!0),i=this.shadowRoot.activeElement,a=!!(t?.shiftKey||this.game?.keys?.ShiftLeft||this.game?.keys?.ShiftRight),o=/^(Arrow|Key[WASD]$)/.test(e);if((o||e===`Tab`||e===`KeyQ`||e===`KeyE`)&&this.classList.add(`kbd`),this.confirmOpen){if(e===`Escape`||e===`GamepadB`)return this.closeConfirm(),r();if(o){let e=[...this.el.confirm.querySelectorAll(`button`)];return e[(e.indexOf(i)+1)%2].focus(),r()}return e===`Enter`||e===`Space`||e===`NumpadEnter`?n&&i?.tagName===`BUTTON`?!0:((i?.closest(`.confirm`)?i:this.el.confirm.querySelector(`[data-confirm=keep]`)).click(),r()):r()}switch(e){case`Tab`:return this.setCategory(this.category+(a?-1:1),!0),r();case`KeyQ`:return this.setCategory(this.category-1,!0),r();case`KeyE`:return this.setCategory(this.category+1,!0),r();case`KeyR`:case`GamepadX`:return this.randomize(),r();case`KeyZ`:return this.undo(),r();case`KeyC`:return this.setCamera(this.cameraChoice===`bust`?`full`:`bust`,!0),r();case`GamepadY`:return this.save(),r();case`Escape`:return this.back(),r();case`Enter`:case`NumpadEnter`:case`Space`:return e!==`Space`&&a?(this.save(),r()):i&&i!==this.el.stage&&i.matches(`button`)?n?!0:(i.click(),r()):(e!==`Space`&&this.save(),r());default:}if(o){let t={ArrowUp:`up`,KeyW:`up`,ArrowDown:`down`,KeyS:`down`,ArrowLeft:`left`,KeyA:`left`,ArrowRight:`right`,KeyD:`right`}[e];return i===this.el.stage&&(t===`left`||t===`right`)?(this.setYaw(this.yawTarget+(t===`left`?-.25:.25)),r()):(this.moveFocus(t,i),r())}return!1}navItems(){let e=this.shadowRoot;return[...this.el.tabs,...e.querySelectorAll(`.content .opt, .content input, .content .play`),this.el.back,this.el.save].filter(e=>e.offsetParent!==null)}moveFocus(e,t){let n=this.navItems();if(!t||!n.includes(t)){let e=this.el.content.querySelector(`.opt[aria-checked="true"]`)||n.find(e=>e.classList.contains(`opt`))||this.el.tabs[this.category];return this.focusNav(e)}if(t.classList.contains(`tab`)&&(e===`left`||e===`right`)){this.setCategory(this.category+(e===`left`?-1:1),!0,!0);return}let r=t.getBoundingClientRect(),i=r.left+r.width/2,a=r.top+r.height/2,o=null,s=1/0;for(let r of n){if(r===t)continue;let n=r.getBoundingClientRect(),c=n.left+n.width/2-i,l=n.top+n.height/2-a,u={up:-l,down:l,left:-c,right:c}[e],d=Math.abs(e===`up`||e===`down`?c:l);if(u<=4||d>u*1.2+12)continue;let f=u+d*2.2;f<s&&(s=f,o=r)}o&&(this.focusNav(o),this.sfx(`menuNav`))}focusNav(e){e&&(e.focus({preventScroll:!0}),e.scrollIntoView?.({block:`nearest`,behavior:P()?`auto`:`smooth`}))}effective(){let e=this.pick(this.character);return this.preview&&(e[this.preview.key]=this.preview.value),e}setPreview(e){if(!(e&&this.preview&&e.key===this.preview.key&&e.value===this.preview.value||!e&&!this.preview)){if(e&&this.character[e.key]===e.value&&(e=null),clearTimeout(this._previewClear),!e){this._previewClear=setTimeout(()=>{this.preview=null,this.queueRender()},70);return}this.preview=e,this.queueRender()}}choose(e,t,r){let i=this.sectionFor(e),a=i?.data?.[t];if(!a)return;let o=n(e,t,this.unlockCtx());if(!o.unlocked){r?.classList.remove(`shake`),r?.offsetWidth,r?.classList.add(`shake`),this.toast(`${a.name} locked · ${o.hint}`),this.sfx(`menuNav`);return}if(this.character[e]===t){this.preview=null;return}this.commit({[e]:t}),this.sfx(`menuSelect`),this.announce(`${i.title}: ${a.name}`)}commit(e){this.el.save.classList.remove(`ready`);let t=this.character,n=this.pick(t);return Object.keys(e).some(n=>t[n]!==e[n])?(this.undoStack.push(n),this.undoStack.length>60&&this.undoStack.shift(),Object.assign(t,e),this.preview=null,this.renderAll(!1),!0):!1}undo(){let e=this.undoStack.pop();if(!e)return this.sfx(`menuNav`);Object.assign(this.character,e),this.preview=null,this.sfx(`menuSelect`),this.announce(`Undone`),this.renderAll(!1)}reset(){let e={};for(let t of S)e[t]=c[t];this.commit(e)&&(this.sfx(`menuSelect`),this.toast(`Reset to standard issue`))}randomize(){let e=e=>Math.floor(Math.random()*e),t={colorIndex:e(f.length),skinToneIndex:e(u.length),hairIndex:e(y.length),eyeIndex:e(g.length),armorIndex:e(b.length),helmetIndex:e(l.length),visorIndex:e(_.length),shoulderIndex:e(x.length),badgeIndex:Math.random()<.15?0:1+e(v.length-1),weaponSkinIndex:e(d.length)};b[t.armorIndex].id===`stealth`&&(Math.random()<.6&&(t.visorIndex=Math.random()<.5?1:2),Math.random()<.6&&(t.weaponSkinIndex=1)),u[t.skinToneIndex].id===`synthetic`&&Math.random()<.7&&(t.eyeIndex=Math.random()<.5?5:1);let r=this.unlockCtx();for(let i of Object.keys(o)){let a=o[i].table.map((e,t)=>t).filter(e=>n(i,e,r).unlocked);i===`loadoutIndex`?t.loadoutIndex=a[e(a.length)]??this.character.loadoutIndex:a.includes(t[i])||(t[i]=a[e(a.length)]??0)}this.commit(t),this.sfx(`menuConfirm`),this.flashStage(),this.announce(`Randomized appearance`)}presetLock(e){let t=this.unlockCtx();for(let[r,i]of Object.entries(D[e].ch)){if(!o[r])continue;let e=n(r,i,t);if(!e.unlocked)return e}return null}applyPreset(e){let t=D[e];if(!t)return;let n=this.presetLock(e);if(n){this.toast(`${t.name} locked · ${n.hint}`),this.sfx(`menuNav`);return}this.commit({...t.ch}),this.sfx(`menuConfirm`),this.flashStage(),this.toast(`${t.name} preset applied`)}onName(e){let t=e.value.slice(0,16);t!==e.value&&(e.value=t);let n=this.nameError(t),r=this.shadowRoot.querySelector(`.err`);if(e.setAttribute(`aria-invalid`,String(!!n)),r&&(r.textContent=n||`Letters, numbers, space . _ ' -`,r.classList.toggle(`ok`,!n)),this.shadowRoot.querySelector(`.count`).textContent=`${t.length}/16`,!n&&t.trim()!==this.character.name){let e=performance.now();(!this._nameEditAt||e-this._nameEditAt>1200)&&this.undoStack.push(this.pick(this.character)),this._nameEditAt=e,this.character.name=t.trim()}this.el.save.setAttribute(`aria-disabled`,String(!!n)),this.el.undo.disabled=this.undoStack.length===0,this.renderHeader()}nameError(e){let t=e.trim();return t?w.test(t)?``:`Use letters, numbers, space . _ ' -`:`Callsign required`}playVoice(e,t){let n=m[e],r=this.game?.audio;try{r?.init?.(),r?.resume?.(),r?.playerGrunt?.(n,`hurt`),setTimeout(()=>r?.playerGrunt?.(n,`slide`),220)}catch{}t.classList.remove(`ping`),t.offsetWidth,t.classList.add(`ping`)}save(){let e=this.shadowRoot.querySelector(`input[name=callsign]`),t=this.nameError(e?e.value:this.character.name||``);if(t){this.setCategory(0),this.toast(t),requestAnimationFrame(()=>this.shadowRoot.querySelector(`input[name=callsign]`)?.focus());return}e&&(this.character.name=e.value.trim()||this.character.name),this.preview=null,this.sfx(`menuConfirm`),this.close(),this.session=!1,this._exitedAt=performance.now(),this.game._exitCreator(!0),this.clearExitGuard()}back(){if(this.isDirty())return this.openConfirm();this.leave()}discard(){Object.assign(this.character,this.snapshot),this.closeConfirm(!0),this.leave()}leave(){let e=this.game;this.sfx(`menuConfirm`),this.close(),this.session=!1,this._exitedAt=performance.now(),e._creatorExitTime=performance.now(),e._exitCreator(!1),this.clearExitGuard()}clearExitGuard(){this.game.state!==`characterCreate`&&(this._exitedAt=0)}openConfirm(){this.confirmOpen=!0,this.el.confirm.hidden=!1,this._confirmReturn=this.shadowRoot.activeElement,this.sfx(`menuNav`),requestAnimationFrame(()=>this.el.confirm.querySelector(`[data-confirm=keep]`).focus())}closeConfirm(e){this.confirmOpen=!1,this.el.confirm.hidden=!0,e||this._confirmReturn?.focus?.()}setCategory(e,t=!1,n=!1){let r=k.length,i=(e%r+r)%r,a=i>this.category?1:-1,o=i!==this.category;this.category=i,this.preview=null;let s=k[i];this.setCamera(s.camera||`full`),this.setPose(s.pose||`idle`),o&&this.sfx(`menuNav`),this.renderAll(!0,o?a:0),t&&(n?this.focusNav(this.el.tabs[i]):this.focusNav(this.el.content.querySelector(`.opt[aria-checked="true"]`)||this.el.content.querySelector(`.opt, input`)))}sectionFor(e){for(let t of k)for(let n of t.sections)if(n.key===e)return n;return null}setCamera(e,t=!1){this.cameraChoice=e,this.setAttribute(`camera`,e),this.shadowRoot.querySelectorAll(`[data-camera]`).forEach(t=>t.setAttribute(`aria-pressed`,String(t.dataset.camera===e))),this.tweenView(e===`bust`?s.bust:s.full),t&&this.sfx(`menuNav`)}setPose(e,t=!1){e===this.poseChoice&&!t||(this.poseChoice=e,this.shadowRoot.querySelectorAll(`[data-pose]`).forEach(t=>t.setAttribute(`aria-pressed`,String(t.dataset.pose===e))),t&&this.sfx(`menuNav`),this.queueRender())}toggleHelmet(){this.helmetOff=!this.helmetOff,this.shadowRoot.querySelector(`[data-helmet-off]`).setAttribute(`aria-pressed`,String(this.helmetOff)),this.sfx(`menuNav`),this.queueRender()}toggleSheet(){let e=this.getAttribute(`sheet`)===`collapsed`;e?this.removeAttribute(`sheet`):this.setAttribute(`sheet`,`collapsed`),this.shadowRoot.querySelector(`.handle`).setAttribute(`aria-expanded`,String(e)),this.sfx(`menuNav`)}setYaw(e,t=!1){this.yawTarget=j(e,-1,1),(t||P())&&(this.yaw=this.yawTarget),this.animate()}tweenView(e){let t=this.view.slice();if(P()||!this.isOpen){this.view=e.slice(),this.applyView();return}this._viewTween={from:t,to:e.slice(),t0:performance.now(),dur:520},this.animate()}animate(){if(this._raf||!this.isOpen)return;let e=t=>{this._raf=0;let n=!1,r=this._viewTween;if(r){let e=j((t-r.t0)/r.dur,0,1),i=e<.5?4*e*e*e:1-(-2*e+2)**3/2;this.view=r.from.map((e,t)=>e+(r.to[t]-e)*i),this.applyView(),e<1?n=!0:this._viewTween=null}Math.abs(this.yaw-this.yawTarget)>.002?(this.yaw+=(this.yawTarget-this.yaw)*.18,n=!0):this.yaw=this.yawTarget,this.applyYaw(),n&&(this._raf=requestAnimationFrame(e))};this._raf=requestAnimationFrame(e)}applyView(){this.el.figure.querySelector(`svg`)?.setAttribute(`viewBox`,this.view.map(e=>e.toFixed(2)).join(` `))}applyYaw(){let e=this.yaw;this.style.setProperty(`--yaw`,e.toFixed(3));let t=this.el.figure.querySelector(`svg`);if(!t)return;let n=(e,n)=>t.querySelector(e)?.setAttribute(`transform`,n),r=(1-Math.abs(e)*.07).toFixed(3);n(`.ag-yaw-back`,`translate(${(-e*7).toFixed(2)} 0) scale(${r} 1)`),n(`.ag-yaw-cape`,`translate(${(-e*4.5).toFixed(2)} 0) scale(${r} 1)`),n(`.ag-yaw-body`,`translate(${(e*.8).toFixed(2)} 0) scale(${r} 1)`),n(`.ag-yaw-head`,`translate(${(e*2.4).toFixed(2)} 0)`),n(`.ag-shadow`,`translate(${(e*3).toFixed(2)} 0)`)}flashStage(){let e=this.el.figure;P()||e.animate?.([{filter:`brightness(1.8)`},{filter:`brightness(1)`}],{duration:320,easing:`ease-out`})}onResize(){this.style.setProperty(`--fig-h`,`${this.el.figureWrap.clientHeight}px`)}queueRender(){this._renderQueued||(this._renderQueued=!0,requestAnimationFrame(()=>{this._renderQueued=!1,this.isOpen&&(this.renderStage(),this.renderHeader(),this.renderPreviewLabels())}))}renderAll(e=!0,t=0){if(!this.isOpen)return;let n=f[this.character.colorIndex]||f[0];this.style.setProperty(`--accent`,n.accent),this.style.setProperty(`--primary`,n.primary),this.el.tabs.forEach((e,t)=>{let n=t===this.category;e.setAttribute(`aria-selected`,String(n)),e.tabIndex=n?0:-1}),e?this.renderContent(t):this.refreshContent(),this.renderPresets(),this.renderStage(),this.renderHeader(),this.el.undo.disabled=this.undoStack.length===0}renderHeader(){let e=this.effective();this.el.callsign.textContent=e.name||`Agent`,this.el.callsign.dataset.text=e.name||`Agent`;let t=p[e.loadoutIndex]||p[0],n=h[e.backstoryIndex||0],r=b[e.armorIndex]||b[0];this.el.sub.innerHTML=`<b>${A(t.name)}</b> · ${A(n.name)} · ${A(r.name)}`;let i=f[e.colorIndex]||f[0];this.style.setProperty(`--accent`,i.accent)}stagePeek(e){if(this.helmetOff)return!0;let t=l[e.helmetIndex]?.id;if(T.has(t))return!1;let n=this.preview?.key||this.previewSection;return E.has(n)}renderStage(){let e=this.effective(),t=this.poseChoice,n=this.stagePeek(e),i=`${t}|${n}|${S.concat([`loadoutIndex`,`backstoryIndex`]).map(t=>e[t]).join(`,`)}`;if(i===this._stageKey)return;this._stageKey=i;let a=this._stageCache.get(i);a||(a=r(e,{pose:t,peek:n,idPrefix:`st-`}),this._stageCache.set(i,a),this._stageCache.size>48&&this._stageCache.delete(this._stageCache.keys().next().value)),this.el.figure.innerHTML=a,this.applyView(),this.applyYaw()}renderPresets(){this.el.presets.querySelectorAll(`.preset`).forEach((e,t)=>{if(!this._presetsBuilt){let n={...c,...D[t].ch};e.querySelector(`.pthumb`).innerHTML=r(n,{view:[-22,-106,44,44],idPrefix:`ps${t}-`,lighting:`flat`})}let n=this.presetLock(t);e.classList.toggle(`locked`,!!n),e.setAttribute(`aria-label`,`Apply ${D[t].name} preset${n?`. Locked: ${n.hint}`:``}`),e.title=n?n.hint:``;let i=e.querySelector(`.pthumb`);i.querySelector(`.lockbadge`)?.remove(),n&&i.insertAdjacentHTML(`beforeend`,`<span class="lockbadge" aria-hidden="true">${O.lock}</span>`)}),this._presetsBuilt=!0}unlockCtx(){return this._unlockCtx||(this._unlockCtx=i(this.game,{fresh:!0})),this._unlockCtx}renderPreviewLabels(){this.shadowRoot.querySelectorAll(`.section[data-key]`).forEach(e=>{let t=this.sectionFor(e.dataset.key),n=e.querySelector(`.cur`);if(!t||!n)return;let r=this.preview?.key===t.key,i=r?this.preview.value:this.character[t.key];n.textContent=t.data[i]?.name||``,n.classList.toggle(`preview`,r)})}renderContent(e=0){let t=k[this.category],n=this.el.content;n.setAttribute(`aria-labelledby`,`tab-${t.id}`),n.innerHTML=t.sections.map(e=>this.sectionHtml(e)).join(``),n.scrollTop=0,e&&(n.style.setProperty(`--swap-dir`,`${e*14}px`),n.classList.remove(`swap`),n.offsetWidth,n.classList.add(`swap`)),this.refreshContent(!0)}sectionHtml(e){if(e.type===`name`){let t=this.character.name||``,n=this.nameError(t);return`<div class="section" data-section="name"><h3><label for="callsign">${e.title}</label></h3>
        <label class="field"><input id="callsign" name="callsign" maxlength="16" autocomplete="off" spellcheck="false" value="${A(t)}" aria-describedby="callsign-err" aria-invalid="${!!n}"><span class="count">${t.length}/16</span></label>
        <p class="err ${n?``:`ok`}" id="callsign-err">${A(n||`Letters, numbers, space . _ ' -`)}</p></div>`}let t=e.data.map((t,n)=>this.optionHtml(e,t,n)).join(``),n=e.kind===`rifle`?`<div class="rifle-hero" aria-hidden="true"><div class="spin"></div></div>`:``;return`<div class="section" data-key="${e.key}"><h3><span>${e.title}</span><span class="cur"></span></h3>${n}<div class="grid ${e.kind}" role="radiogroup" aria-label="${e.title}">${t}</div>${this.lockInfoHtml(e)}</div>`}lockInfoHtml(e){if(!o[e.key]||e.kind===`class`)return``;let t=this.unlockCtx(),r=new Map;return e.data.forEach((i,a)=>{let o=n(e.key,a,t);if(o.unlocked)return;let s=r.get(o.hint)||{st:o,names:[]};s.names.push(i.name),r.set(o.hint,s)}),r.size?`<div class="lockinfo">${[...r.values()].map(({st:e,names:t})=>`<div class="lockrow"><span class="lock caption amber">${O.lock}${A(t.join(`, `))}</span><span>${A(e.hint)}</span><span class="bar" aria-hidden="true"><i style="--w:${Math.round(e.pct*100)}%"></i></span></div>`).join(``)}</div>`:``}optionHtml(e,t,r){let i=o[e.key]?n(e.key,r,this.unlockCtx()):null,s=!!i&&!i.unlocked,c=`class="opt plate${e.kind===`origin`||e.kind===`voice`||e.kind===`class`?` card`:``}${s?` locked`:``}" role="radio" aria-checked="false" data-key="${e.key}" data-idx="${r}" tabindex="-1"`,l=`${t.name}${t.desc?`. ${t.desc}`:``}${t.perk?`. Perk: ${t.perk}`:``}${s?`. Locked: ${i.hint}`:``}`,u=`<span class="tick" aria-hidden="true">${O.check}</span>`,d=t.tier?`<span class="tier" aria-hidden="true">${[1,2,3].map(e=>`<i class="${e<=t.tier?`on`:``}"></i>`).join(``)}&nbsp;${M[t.tier]}</span>`:``;switch(e.kind){case`eye`:return`<button ${c} aria-label="${A(l)}" title="${A(t.name)}"><span class="swatch" style="--c:${t.color}"></span><span class="oname">${A(t.name)}</span></button>`;case`skin`:return`<button ${c} aria-label="${A(l)}" title="${A(t.name)}"><span class="swatch" style="background:radial-gradient(circle at 35% 30%, ${t.color}, ${t.shadow})"></span><span class="oname">${A(t.name)}</span></button>`;case`palette`:return`<button ${c} aria-label="${A(l)}">${u}<span class="pal" aria-hidden="true"><i style="background:linear-gradient(135deg, ${t.primary}, ${t.dark})"></i><i style="background:${t.accent}"></i><i style="background:${t.dark}"></i></span><span class="oname">${A(t.name)}</span></button>`;case`badge`:return`<button ${c} aria-label="${A(l)}" title="${A(t.name)}">${u}<span class="badge-tile" aria-hidden="true">${t.icon?`<svg viewBox="-6.5 -6.5 13 13">${a(t.icon,`var(--accent)`,`#070b11`)}</svg>`:`<span class="oname" style="color:var(--faint)">None</span>`}</span><span class="oname">${A(t.name)}</span></button>`;case`origin`:return`<button ${c} aria-label="${A(l)}">${u}<span class="row"><span class="oname">${A(t.name)}</span></span><span class="desc">${A(t.desc)}</span><span class="perk caption cyan">${A(t.perk)}</span></button>`;case`voice`:return`<div class="voice-row" style="position:relative"><button ${c} aria-label="${A(l)}" style="width:100%;padding-right:60px">${u}<span class="oname">${A(t.name)}</span><span class="desc">${A(t.desc)}</span></button><button class="play" data-voice="${r}" aria-label="Preview ${A(t.name)} voice" style="position:absolute;right:10px;top:50%;transform:translateY(-50%)">${O.play}</button></div>`;case`class`:{let e=N(t).map(e=>{let t=t=>`${Math.round(j(t/e.max,0,1)*100)}%`,n=e.fmt||(e=>Math.round(e)),r=e.value-e.base,i=Math.abs(r)<1e-6?``:r>0?`up`:`down`;return`<span>${e.label}</span><span class="bar"><i style="--w:${t(e.value)}"></i></span><b class="${i}">${n(e.value)}</b>`}).join(``),n=s?`<span class="lock caption amber">${O.lock}Locked</span>`:``,r=s?`<span class="lockprog"><span>${A(i.hint)}</span><span class="bar" style="width:70px" aria-hidden="true"><i style="--w:${Math.round(i.pct*100)}%"></i></span></span>`:``;return`<button ${c} aria-label="${A(l)}" aria-disabled="${s}">${u}<span class="row"><span class="oname">${A(t.name)}</span>${n}</span><span class="desc">${A(t.desc)}</span>${r}<span class="stats" aria-hidden="true">${e}</span></button>`}case`rifle`:return`<button ${c} aria-label="${A(l)}">${u}<span class="thumb" data-thumb="rifle"></span><span class="oname">${A(t.name)}</span></button>`;default:return`<button ${c} aria-label="${A(l)}"${s?` aria-disabled="true" title="${A(i.hint)}"`:``}>${u}${s?`<span class="lockbadge" aria-hidden="true">${O.lock}</span>`:``}<span class="thumb" data-thumb="${e.kind}"></span><span class="oname">${A(t.name)}</span>${d}</button>`}}refreshContent(e=!1){let n=this.character,i=S.map(e=>n[e]).join(`,`);this.shadowRoot.querySelectorAll(`.content .opt[data-key]`).forEach(a=>{let o=a.dataset.key,c=Number(a.dataset.idx);a.setAttribute(`aria-checked`,String(n[o]===c));let l=a.querySelector(`.thumb[data-thumb]`);if(!l||!e&&l.dataset.sig===i)return;l.dataset.sig=i;let u={...n,[o]:c},d=`t${this._thumbSeq++}-`;switch(l.dataset.thumb){case`head`:l.innerHTML=r(u,{headOnly:!0,idPrefix:d});break;case`hair`:l.innerHTML=r(u,{headOnly:!0,peek:!0,idPrefix:d});break;case`torso`:l.innerHTML=r(u,{view:s.torso,idPrefix:d,lighting:`flat`});break;case`rifle`:l.innerHTML=t(u,{idPrefix:d});break;default:}});let a=this.shadowRoot.querySelector(`.rifle-hero .spin`);a&&(e||a.dataset.sig!==i)&&(a.dataset.sig=i,a.innerHTML=t(n,{idPrefix:`rh${this._thumbSeq++}-`}));let o=this.shadowRoot.querySelector(`input[name=callsign]`);o&&this.shadowRoot.activeElement!==o&&o.value!==(n.name||``)&&(o.value=n.name||``),this.el.save.setAttribute(`aria-disabled`,String(!!this.nameError(n.name||``))),this.renderPreviewLabels(),this.el.undo.disabled=this.undoStack.length===0}toast(e){let t=this.el.toast;t.textContent=e,t.classList.add(`on`),clearTimeout(this._toastT),this._toastT=setTimeout(()=>t.classList.remove(`on`),2200)}announce(e){this.el.live.textContent=e}};customElements.get(`agent-showroom`)||customElements.define(`agent-showroom`,I);function L(e){let t=document.querySelector(`agent-showroom`);return t||(t=document.createElement(`agent-showroom`),document.body.appendChild(t)),t.game=e,t}export{L as mountShowroom};