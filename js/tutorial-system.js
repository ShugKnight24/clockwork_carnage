...
export class TutorialSystem {
  ...
  advanceStep() {
    ...
    const autoOpenDoors = {
      ...
      7: doorMap.door3,   // Step 7 (Sprint) → open Fitness Center
      9: doorMap.door4,   // Step 9 (Chrono) → open Supervisor Office
      11: doorMap.door5,  // Step 11 (Alert) → open Combat Arena
    };
  ...
  update(dt) {
    ...
    switch (this.step) {
      ...
      case 9: // CHRONO SHIFT
        g.objectiveWaypoint = { x: 45.5, y: 21.5 };
        if (elapsed > 0.1 && this.chronoUsed) this.advanceStep();
        break;

      case 10: // RESUPPLY
        g.objectiveWaypoint = { x: 46.5, y: 21.5 };
  ...
  executeCompletionChoice(choice) {
    ...
    this.showCompletionMenu = false;
    switch (choice) {
      case 0: // Continue Training (sandbox step 15)
        g.state = GameState.PLAYING;
        this.advanceStep();
        g.audio.startTrack("campaign", 130);
        g.audio.startAmbient("industrial");
      ...
      case 1: // Begin Campaign
        ...
        g.startCutscene("the_hunt_begins", () => {
